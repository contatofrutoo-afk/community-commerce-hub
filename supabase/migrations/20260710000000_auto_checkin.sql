...
-- 1. Vincula b2c_customers ao auth user (se ainda não existir)
ALTER TABLE public.b2c_customers
...
-- 2. RPC: auto_checkin
--    - Upsert b2c_customers (find by auth_user_id + company, or create)
--    - Cria check-in apenas se último check-in foi há mais de 4 horas
...
DECLARE
  ...
  v_table_name    text;
BEGIN
  -- Normaliza tables vazios para NULL
  v_table_id   := nullif(p_table_id, '');
  v_table_name := nullif(p_table_name, '');

  -- Upsert b2c_customers vinculado ao auth_user
  ...
  -- Verifica último check-in deste customer nesta empresa
  ...
  -- Se há check-in recente dentro do cooldown, não duplica
  IF v_last_checkin IS NOT NULL AND (v_now - v_last_checkin) < v_cooldown THEN
    RETURN false;
  END IF;

  -- Day of week em português
...
GRANT EXECUTE ON FUNCTION public.auto_checkin(uuid, uuid, text, text, text, text) TO anon, authenticated;
