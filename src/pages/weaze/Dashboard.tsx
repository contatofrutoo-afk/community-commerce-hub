...
export default function WeazeDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (activeCompanyIds.length > 0) {
        ...
        const activeNames = new Map((activeTenantData ?? []).map((t: any) => [t.id, t.name]));

        for (const cid of activeCompanyIds.slice(0, 20)) {
          const [{ count: posts }, { count: checkins }] = await Promise.all([
            supabase.from("posts").select("*", { count: "exact", head: true }).eq("tenant_id", cid),
          ...
          if ((posts ?? 0) === 0 && (checkins ?? 0) === 0) {
            inactiveCompanies.push(name);
          }
        }
