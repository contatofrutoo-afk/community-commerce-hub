...
import { TrendingUp, TrendingDown, AlertTriangle, ArrowLeft, Users, Heart, MessageCircle, Eye, Target } from "lucide-react";
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const dPrev = new Date(now.getTime() - prevPeriodDays * 86400_000).toISOString();
      const d1 = new Date(now.getTime() - 86400_000).toISOString();

      const [{ count: members }, { count: newMembers }, { data: interactions }, { data: interactionsPrev }, { data: posts }] = await Promise.all([
        ...
        supabase.from("interactions").select("user_id, created_at, action_type, post_id").eq("tenant_id", tenant.id).gte("created_at", dCurrent),
        supabase.from("interactions").select("user_id, created_at").eq("tenant_id", tenant.id).gte("created_at", dPrev).lt("created_at", dCurrent),
      ...
      const avgEngagement = topPosts.length > 0 
        ? topPosts.reduce((a, b) => a + b.engagement, 0) / topPosts.length 
        : 0;

      const ctaStats: CtaStats[] = [];
      const ctaTypes = ["BUY", "SCHEDULE", "QUOTE", "REGISTER"];
      for (const type of ctaTypes) {
        const ctaPosts = (posts ?? []).filter((p: any) => p.cta_clicks > 0 || p.conversions > 0);
        const clicks = ctaPosts.reduce((a: number, p: any) => a + (p.cta_clicks ?? 0), 0);
        const conversions = ctaPosts.reduce((a: number, p: any) => a + (p.conversions ?? 0), 0);
      ...
      const interactionsChart = Object.entries(buckets).reverse().map(([date, count]) => ({
        date: date.slice(5),
        count,
      }));

      const currentCount = (interactions ?? []).length;
      const prevCount = (interactionsPrev ?? []).length;
      ...
      if (mau === 0 && (posts?.length ?? 0) > 0) alerts.push("Sem usuários ativos no período");
      if ((posts?.length ?? 0) === 0) alerts.push("Nenhum post publicado no período");
      ...
      setData({
        ...
        messagesSent: 0,
        avgResponseTime: 0,
        interactions30: interactionsChart,
        growth30,
        alerts,
      });
    })();
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {data.ctaStats.length > 0 ? data.ctaStats.map((cta) => (
